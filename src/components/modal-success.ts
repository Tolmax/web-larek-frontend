...
export class ModalSucess extends Modal {
    ...
    // private _apiResponseCreateOrder: IApiResponseCreateOrder

	constructor(
		events: EventEmitter,
		templateSelector: string,
        // apiResponseCreateOrder: IApiResponseCreateOrder,
	) {
		super();
		this._templateSelector = templateSelector;
		this._events = events;
        // this._apiResponseCreateOrder = apiResponseCreateOrder;
	}
	...
	private _createHTMLElement(order: IApiResponseCreateOrder): HTMLElement {
		...
		this._nodes = {
			...
			modalSuccessDescription: modalSucess.querySelector('.order-success__description'),
			modalSuccessCloseButton: modalSucess.querySelector('.order-success__close'),
		};

        this._nodes.modalSuccessDescription.textContent = 'Списано ' + `${order.total}` + ' синапсов',

		this._nodes.modalSuccessCloseButton.addEventListener('click', () => {
            // this._events.emit('sucess:closed');
            this._close();
        });
